...
int ClientIO::init_env()
{
  ...
  for (auto header = headers.begin(); header != headers.end(); ++header) {
    ...
    if (field == beast::http::field::content_length) {
      env_map["CONTENT_LENGTH"] = value.to_string();
      continue;
    }
    if (field == beast::http::field::content_type) {
      env_map["CONTENT_TYPE"] = value.to_string();
      continue;
    }
    ...
    *dest = '\0';
    env_map[buf] = value.to_string();
  }

  int major = request.version() / 10;
  int minor = request.version() % 10;
  env_map["HTTP_VERSION"] = std::to_string(major) + '.' + std::to_string(minor);

  env_map["REQUEST_METHOD"] = request.method_string().to_string();
  ...
  if (pos != uri.npos) {
    ...
    env_map["QUERY_STRING"] = query.to_string();
    uri = uri.substr(0, pos);
  }
  env_map["SCRIPT_URI"] = uri.to_string();

  env_map["REQUEST_URI"] = request.target().to_string();

  char port_buf[16];
  snprintf(port_buf, sizeof(port_buf), "%d", local_endpoint.port());
  env_map["SERVER_PORT"] = port_buf;
  if (is_ssl) {
    env_map["SERVER_PORT_SECURE"] = port_buf;
  }
  env_map["REMOTE_ADDR"] = remote_endpoint.address().to_string();
